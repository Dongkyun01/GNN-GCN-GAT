...
# ✅ 최적 threshold + F1-score 변화 곡선 계산
def find_best_threshold(labels, preds):
    ...
    for t in torch.linspace(0.01, 0.99, steps=99):
        ...
        f1_curve[round(t.item(), 2)] = f1  # 예: {0.1: 0.33, 0.2: 0.41, ...}
        if f1 > best_f1:
            best_f1 = f1
...
def eval_gat(model, test_loader, edge_index, device, save_path="results/gat.json"):
    model.eval()
    ...
    print(f"[GAT] Eval AUC: {auc:.4f} | Hit@1: {hit:.4f} | Precision: {precision:.4f} | Recall: {recall:.4f} | Best Thresh: {best_threshold:.2f}")
